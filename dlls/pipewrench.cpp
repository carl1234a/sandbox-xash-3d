...
{
        pev->classname = MAKE_STRING( "weapon_pipewrench" ); // hack to allow for old names
	Precache();
...
int CPipeWrench::Swing(int fFirst)
{
	...
	if ( tr.flFraction >= 1.0 )
	{
		...
		switch ( ((m_iSwing++) % 1) )
		{
		case 0: EMIT_SOUND( ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/pwrench_miss1.wav", 1, ATTN_NORM); break;
		case 1: EMIT_SOUND( ENT(m_pPlayer->pev), CHAN_ITEM, "weapons/pwrench_miss2.wav", 1, ATTN_NORM); break;
		}
